-- from https://github.com/Pytness/dotfiles originally
...
ASCII_IMAGES_FOLDER = os.getenv("HOME") .. "/.config/lvim/static/splashes"
...
if is_colored_image(random_image) then
    ...
    command = command .. os.getenv("HOME") .. "/.config/lvim/static/animated_lolcat.sh "
end

-- chance to get a bonsai
if false then -- math.random(1, 5) > 3 then
    command = "bash " .. os.getenv("HOME") .. "/.config/lvim/static/cbonsai.sh"
else
...
alphadash.section.buttons.val = {
	...
	button("SPC s t", "󰊄  Find Text", "<CMD>Telescope live_grep<CR>"),
...
-- alphadash.section.buttons.val = lvim.builtin.alpha.dashboard.section.buttons.val
...
local function footer()
	...
	local text_table = text.align_center({ width = 0 }, {
		datetime .. "  󱐥 " .. custom_plugins .. " custom plugins" .. nvim_version_info,
	}, 0.5)
